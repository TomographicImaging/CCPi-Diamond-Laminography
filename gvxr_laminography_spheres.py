...
# %% Get a sample
radius = 20
gvxr.makeSphere(simulation_name, 10, 10, 20, "mm")
gvxr.addPolygonMeshAsInnerSurface(simulation_name)
gvxr.translateNode(simulation_name, 20, 0, -15, "mm")
gvxr.applyCurrentLocalTransformation(simulation_name)

gvxr.makeSphere("sphere2", 10, 10, 15, "mm")
gvxr.translateNode("sphere2", -10, 0, 5, "mm")
gvxr.applyCurrentLocalTransformation("sphere2")
gvxr.addMesh(simulation_name, "sphere2")

gvxr.saveSTLfile(simulation_name, simulation_name + ".stl")
# %%
gvxr.removePolygonMeshesFromSceneGraph()
gvxr.loadMeshFile(simulation_name, simulation_name + ".stl", "mm")
gvxr.moveToCentre(simulation_name)
gvxr.applyCurrentLocalTransformation(simulation_name)
# Choose a density for the sample Carbon (Z number: 6, symbol: C)
gvxr.setElement(simulation_name, "C")
gvxr.setElement("sphere2", "C")
...
gvxr.rotateNode(simulation_name, tilt, 1, 0, 0)

print("Compute an X-ray image")
x_ray_image = np.array(gvxr.computeXRayImage()) / gvxr.getTotalEnergyWithDetectorResponse()
show2D(x_ray_image)

# %% Check we can still see the sample as we rotate around the CT rotation axis
...
# %% Calcualte the rotated axis and simulate CT scan
axis = np.array([1, 0, 0])
...
gvxr.computeCTAcquisition(os.path.join(output_path, simulation_name + "_projections_" + str(number_of_projections) + "_tilt_" + str(tilt)), # the path where the X-ray projections will be saved.
                          ...
                          os.path.join(output_path, simulation_name + "_screenshots_" + str(number_of_projections)+ "_tilt_" + str(tilt)), # the path where the screenshots will be saved.
...
json_fname = os.path.join(output_path, simulation_name + "_simulation_" + str(number_of_projections) + "_tilt_" +str(tilt)+ ".json")
